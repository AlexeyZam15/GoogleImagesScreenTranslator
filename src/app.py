...
class ScreenshotTranslatorApp:
    ...
    def _on_lang_enter(self, event):
        """Обработчик нажатия Enter - выбирает первый подходящий вариант"""
    ...
    def _apply_language(self, selected):
        ...
        if self.translator:
            self.translator.update_target_language(lang_code)
            self.logger.info(f"Язык перевода обновлен на: {lang_code}")
